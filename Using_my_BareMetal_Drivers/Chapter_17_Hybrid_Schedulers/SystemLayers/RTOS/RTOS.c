...
void hSCH_Init_T0(void){
    Gpt_SetCallBackFunction(RTOS_Used_Timer_Channel, hSCH_Update, Gpt_InterruptOverFlow);
...
-*------------------------------------------------------------------*/
uint8 hSCH_Add_Task(void (*pFunction)(void), const uint32 DELAY, const uint32 PERIOD, uint8 co_op){
    uint8 Index = 0;
...
-*------------------------------------------------------------------*/
uint8 hSCH_Delete_Task(const uint8 TASK_INDEX)
{
    uint8 Return_code;
